...
export class MaxbetBettingDetailsComponent implements OnInit {
    ...
    BettingDetails() {
        ...
        this.adminService.add<any>(customer.MaxbetBettingDetails, Model).subscribe(res => {
            ...
            if (res.data.Data.BetDetails.length > 0) {
                this.BettingDetailList = res.data.Data.BetDetails;
                this.BettingNumberDetailList = res.data.Data.BetNumberDetails;
                this.setColumn();
                if (res.data.Data.BetDetails.length > 0) {
                    res.data.Data.BetDetails.forEach(el => {
                        this.rows.push({
                ...
                if (res.data.Data.BetNumberDetails.length > 0) {
                    res.data.Data.BetNumberDetails.forEach(el => {
