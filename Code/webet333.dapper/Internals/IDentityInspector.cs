...
namespace Webet333.dapper.Internals
{
    public class IDentityInspector<TEntity> : IIDentityInspector<TEntity> where TEntity : class
    {
        ...
        {
            string result = null;
            string qry = $"select COLUMN_NAME from INFORMATION_SCHEMA.COLUMNS where COLUMNPROPERTY(object_id(TABLE_SCHEMA + '.' + TABLE_NAME), COLUMN_NAME, 'IsIdentity') = 1 and TABLE_NAME = @TableName order by TABLE_NAME";
            try
            {
