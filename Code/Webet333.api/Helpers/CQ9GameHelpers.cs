...
namespace Webet333.api.Helpers
{
    public class CQ9GameHelpers : IDisposable
    {
        ...
        internal static async Task<CQ9GetLoginURLResponse> CallGetLoginURLAPI(string Token, string Language)
        {
            string model = $"usertoken={Token}&" +
                $"lang={Language}";

            string temp = await CallPostAPIAsync(CQ9Const.EndPoint.GameLobby, model);
