...
namespace Webet333.api.Controllers
{
    ...
    public class TransferMoneyController : BaseController
    {
        ...
        public async Task<IActionResult> BalanceTransfer([FromBody] TransferMoneyRequest request)
        {
            ...
            var responseId = await ApiLogsManager.APITransactionLogsInsert(
            ...
            {
                userDetails = await transfer_helper.UserDetails(
            ...
            if (userDetails.FromWalletIsMaintenance == true)
            {
                await ApiLogsManager.APITransactionLogsInsert(
                    ...
                    {
                        Id = Id,
                        Response = Localizer["error_game_maintenance"].Value
                    });
            ...
            if (userDetails.FromWalletName == "Main Wallet")
                if (userDetails.MainWalletBalance < request.Amount)
                {
                    await ApiLogsManager.APITransactionLogsInsert(
                        ...
                        {
                            Id = Id,
                            Response = Localizer["error_insufficient_balance"].Value
                        });
            ...
            using (var transferMoney_helper = new TransferMoneyHelpers(Connection, Localizer))
            {
                ...
                var WithdrawResponse = await transferMoney_helper.WithdrawFromWallet(
                    userDetails,
                    userDetails.FromWalletName,
                    request.Amount,
                    request.UserId.ToString(),
                    _hostingEnvironment);

                if (string.IsNullOrEmpty(WithdrawResponse.ErrorMessage) &&
                    ...
                    string.IsNullOrEmpty(WithdrawResponse.GameResponse))
                {
                    ...
                    var DepositResponse = await transferMoney_helper.DepositInWallet(
                        userDetails,
                        userDetails.ToWalletName,
                        request.Amount,
                        request.UserId.ToString(),
                        _hostingEnvironment);

                    if (string.IsNullOrEmpty(DepositResponse.ErrorMessage) &&
                        string.IsNullOrEmpty(DepositResponse.GameName) &&
                        string.IsNullOrEmpty(DepositResponse.GameResponse))
                    {
                        if (Role == RoleConst.Users)
                            await transferMoney_helper.Transfer(
                                request.UserId.ToString(),
                                request.FromWalletId.ToString(),
                                request.ToWalletId.ToString(),
                                request.Amount,
                                request.UserId.ToString(),
                                StatusConsts.Approved,
                                request.UserId.ToString());
                        else
                            await transferMoney_helper.Transfer(
                                request.UserId.ToString(),
                        ...
                        await ApiLogsManager.APITransactionLogsInsert(
                            ...
                            {
                                Id = Id,
                                Response = Localizer["ok_response_success"].Value,
                                FromWalletResponse = JsonConvert.SerializeObject(WithdrawResponse),
                                ToWalletResponse = JsonConvert.SerializeObject(DepositResponse)
                            });
                    }
                    else
                    {
                        var DepositFailedResponse = await transferMoney_helper.DepositInWallet(
                            userDetails,
                            userDetails.FromWalletName,
                            request.Amount,
                            request.UserId.ToString(),
                        ...
                        if (string.IsNullOrEmpty(DepositFailedResponse.ErrorMessage) &&
                            string.IsNullOrEmpty(DepositFailedResponse.GameName) &&
                            string.IsNullOrEmpty(DepositFailedResponse.GameResponse))
                        {
                        }
                        else
                        {
                            //Deposit In Main Wallet and Insert into DB Row
                            await transferMoney_helper.DepositInWallet(
                                userDetails,
                                "Main Wallet",
                                request.Amount,
                                request.UserId.ToString(),
                            ...
                            await ApiLogsManager.APITransactionLogsInsert(
                                ...
                                {
                                    Id = Id,
                                    Response = "Completed Transaction Is Failed So We Add Money in Main Wallet",
                                    FromWalletResponse = JsonConvert.SerializeObject(WithdrawResponse),
                                    ToWalletResponse = JsonConvert.SerializeObject(DepositResponse)
                                });
                        ...
                        await ApiLogsManager.APITransactionLogsInsert(
                            ...
                            {
                                Id = Id,
                                Response = DepositResponse.GameName + " Deposit Api Failed \n" + DepositResponse.ErrorMessage,
                                FromWalletResponse = JsonConvert.SerializeObject(WithdrawResponse),
                                ToWalletResponse = JsonConvert.SerializeObject(DepositResponse)
                            });
                ...
                else
                {
                    ...
                    await ApiLogsManager.APITransactionLogsInsert(
                        ...
                        {
                            Id = Id,
                            Response = WithdrawResponse.GameName + " Withdraw Api Failed \n" + WithdrawResponse.ErrorMessage,
                            FromWalletResponse = JsonConvert.SerializeObject(WithdrawResponse)
                        });
