...
async function AllInButtonDisable(i) {
    ...
    for (i = 0; i < walletData.data.length; i++) {
        ...
        if (walletData.data[i].walletType == "SBO Wallet" &&
            walletData.data[i].isMaintenance == true) {
            if (window.location.href.toLowerCase().includes('mobile/transfer')) document.getElementById("SBOAllIn").disabled = true;
        }
        else if (walletData.data[i].walletType == "SBO Wallet" &&
            walletData.data[i].isMaintenance == false) {
            if (window.location.href.toLowerCase().includes('mobile/transfer')) document.getElementById("SBOAllIn").disabled = false;
        }
    }
}
        ...
        switch (gamename) {
            ...
            case 'Playtech':
                if (resSelectUser.data.Playtech !== true) {
                    ...
                    //}
                    if (resPlaytech1.data.errorcode == 0) {
                        loginPlaytech();
                    }
                }
                else {
